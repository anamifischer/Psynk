<!DOCTYPE html>
<html lang="pt-BR">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">

    <title>Agenda — Psynk</title>

    <link rel="stylesheet"
          href="https://meyerweb.com/eric/tools/css/reset/reset.css">

    <link rel="stylesheet" href="/app/css/variables.css">
    <link rel="stylesheet" href="/app/css/globals.css">
    <link rel="stylesheet" href="/app/css/agenda.css">
</head>

<body>

    <?php include($_SERVER['DOCUMENT_ROOT'] . "/app/includes/menuAdmin.php"); ?>


    <div class="main agenda">

        <!-- SIDEBAR DA AGENDA -->
        <aside class="painel-filtros">

            <div class="filtro-secao">
                <div class="alternador-view">
                    <button class="botao-view ativo" data-view="mes">
                        Mês
                    </button>

                    <button class="botao-view" data-view="semana">
                        Semana
                    </button>

                    <button class="botao-view" data-view="ano">
                        Ano
                    </button>
                </div>
            </div>


            <div class="filtro-secao">

                <p class="filtro-titulo">
                    Psicólogos
                </p>

                <div
                    class="lista-psicologos"
                    id="lista-psicologos">
                </div>

            </div>


            <div class="filtro-secao">

                <p class="filtro-titulo">
                    Status
                </p>

                <div class="legenda-status">

                    <div class="legenda-item">
                        <span class="legenda-bolinha confirmado"></span>
                        <span>Confirmado</span>
                    </div>

                    <div class="legenda-item">
                        <span class="legenda-bolinha pendente"></span>
                        <span>Pendente</span>
                    </div>

                    <div class="legenda-item">
                        <span class="legenda-bolinha cancelado"></span>
                        <span>Cancelado</span>
                    </div>

                </div>

            </div>

        </aside>


        <!-- ÁREA DO CALENDÁRIO -->
        <div class="area-principal">

            <header class="barra-topo">

                <div class="topo-esquerda">

                    <button
                        class="botao-nav"
                        id="btn-anterior">
                        ‹
                    </button>

                    <h2
                        class="periodo-atual"
                        id="periodo-atual">
                    </h2>

                    <button
                        class="botao-nav"
                        id="btn-proximo">
                        ›
                    </button>

                </div>


                <div class="topo-direita">

                    <button
                        class="botao-nova-consulta"
                        onclick="abrirNovaConsulta()">
                        + Nova consulta
                    </button>

                </div>

            </header>


            <div
                class="cabecalho-semana"
                id="cabecalho-semana">

                <span>Dom</span>
                <span>Seg</span>
                <span>Ter</span>
                <span>Qua</span>
                <span>Qui</span>
                <span>Sex</span>
                <span>Sáb</span>

            </div>


            <div
                class="grade-calendario"
                id="grade-calendario">
            </div>

        </div>

    </div>


    <!-- PAINEL DE DETALHES -->
    <div
        class="painel-detalhe"
        id="painel-detalhe">

        <div class="detalhe-cabecalho">

            <h3 id="detalhe-titulo">
                Detalhes
            </h3>

            <button id="fechar-detalhe">
                ✕
            </button>

        </div>

        <div
            class="detalhe-conteudo"
            id="detalhe-conteudo">
        </div>

    </div>


    <script src="/app/js/agenda.js"></script>

</body>
</html>